...
#region grDate
        ...
        protected void grCC_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            try
            {
                ...
                //DataTable dtPro = General.IncarcaDT($@"SELECT ""Id"", ""Denumire"" FROM ""tblProiecte"" WHERE {General.TruncateDateAsString("\"DataInceput\"")} <= {General.ToDataUniv(Convert.ToDateTime(lst[1]))} AND {General.ToDataUniv(Convert.ToDateTime(lst[1]))} <= {General.TruncateDateAsString("\"DataSfarsit\"")} ", null);
        ...
        protected void cmbSub_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
    ...
    //        cmb.Value = gridContainer.KeyValue;
    ...
    //            case "cmbPro":
    //                {
    //                    ASPxComboBox cmbSub = ((ASPxComboBox)grCC.FindEditRowCellTemplateControl(grCC.Columns["Category3ID"] as GridViewDataComboBoxColumn, "Cat3"));
    //                }
    //                break;
    //        }
