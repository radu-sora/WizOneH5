    ...
    public partial class Lista : System.Web.UI.Page
    {
        ...
        protected void grDate_CustomButtonInitialize(object sender, ASPxGridViewCustomButtonEventArgs e)
        {
            try
            {
                ...
                //if (e.ButtonID == "btnAtasamentAdd")
                //{
                //    object row = ((ASPxGridView)sender).GetRow(e.VisibleIndex);
                //    if (row == null) return;
                //    string id = ((DataRowView)row)["AdaugaAtasament"].ToString();
                //    if (id == "1")
                //        e.Enabled = true;
                //    else
                //        e.Enabled = false;
        ...
        protected void grDate_CommandButtonInitialize(object sender, ASPxGridViewCommandButtonEventArgs e)
        {
            try
            {
                if (e.ButtonType == ColumnCommandButtonType.Edit)
                {
                    object row = ((ASPxGridView)sender).GetRow(e.VisibleIndex);
                    if (row == null) return;
                    int id = Convert.ToInt32(General.Nz(((DataRowView)row)["IdStare"],1));
                    if (id == 1 || id == 2 || id == 4)
                        e.Enabled = true;
                    else
                        e.Enabled = false;
        ...
        protected void grDate_HtmlEditFormCreated(object sender, ASPxGridViewEditFormEventArgs e)
        {
            try
            {
                object[] obj = grDate.GetRowValues(grDate.FocusedRowIndex, new string[] { "Compensare", "CompensareBanca", "CompensarePlata", "CompensareBancaDenumire", "CompensarePlataDenumire", "Inlocuitor", "TrimiteLa", "AdaugaAtasament" }) as object[];
        ...
        protected void btnDocUpload_FileUploadComplete(object sender, DevExpress.Web.FileUploadCompleteEventArgs e)
        {
            try
            {
                ...
                string sqlFis = $@"if ((SELECT COUNT(*) FROM ""tblFisiere"" WHERE ""Tabela""=@1 AND ""Id""=@2 AND ""EsteCerere""=0) = 0)
