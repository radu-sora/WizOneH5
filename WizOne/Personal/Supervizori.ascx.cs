    ...
    public partial class Supervizori : System.Web.UI.UserControl
    {
        ...
        protected void grDateSupervizori_RowUpdating(object sender, DevExpress.Web.Data.ASPxDataUpdatingEventArgs e)
        {
            try
            {
                ...
                {
                    if (!col.AutoIncrement && grDateSupervizori.Columns[col.ColumnName].Visible)
                    {
                    ...
                    if (!col.AutoIncrement)
                    {
                        switch (col.ColumnName.ToUpper())
                        {
                            ...
                            case "IDAUTO":
                                ...
                                break;
                            case "DATAINCEPUT":
                                row[x] = DateTime.Now.Date.ToShortDateString();
                                break;
                            case "DATASFARSIT":
                                row[x] = new DateTime(2100, 1, 1).ToShortDateString();
                                break;
                            case "USER_NO":
