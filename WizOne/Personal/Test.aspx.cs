...
namespace WizOne.Personal
{
    public partial class Test : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string[] arrCampuri = { "F10003", "F10008", "F10009" };

            DataTable dt = General.IncarcaDT("SELECT * FROM F100 A INNER JOIN F1001 B ON A.F10003=B.F10003 WHERE A.F10003=460", null);
            if (dt == null || dt.Rows.Count == 0) return;
            DataRow dr = dt.Rows[0];

            for(int i=0; i < arrCampuri.Length; i++)
            {
                HtmlGenericControl div = CreazaDiv("row");
                HtmlGenericControl colLbl = CreazaDiv("col-lg-6");
                ASPxLabel lbl = new ASPxLabel();
                lbl.Text = Dami.TraduCuvant("Marca");
                HtmlGenericControl colTxt = CreazaDiv("col-lg-6");
                ASPxTextBox txt = new ASPxTextBox();
                txt.ID = "ctl_" + arrCampuri[i];
                txt.Text = General.Nz(dr[arrCampuri[i]], "").ToString();
                colLbl.Controls.Add(lbl);
                colTxt.Controls.Add(txt);
                div.Controls.Add(colLbl);
                div.Controls.Add(colTxt);

                tabMain.TabPages[0].Controls.Add(div);
            }
        ...
        HtmlGenericControl CreazaDiv(string atrClass="")
        {
            HtmlGenericControl div = new HtmlGenericControl("div");
            if (atrClass != "")
                div.Attributes["class"] = atrClass;
            return div;
        }
        ...
        private void IncarcaGrid()
        {
            ...
            string sqlFinal = "SELECT * FROM \"F100Contacte\" WHERE F10003 = " + Session["User_Marca"].ToString();
