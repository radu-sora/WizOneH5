...
namespace WizOne.Pontaj
{
	...
	(select ',' + X.DenumireScurta from ( 
                                ...
                                (X.""ZileSapt""=(CASE WHEN P.""ZiSapt""<6 AND P.""ZiLibera""=0 THEN 1 ELSE 0 END) 
                                AND X.SD = (CASE WHEN P.""ZiSapt"" = 6 OR P.""ZiSapt"" = 7 THEN 1 ELSE 0 END) 
								AND X.SL = (CASE WHEN P.""ZiSapt"" < 6 AND P.""ZiLibera"" = 1 THEN 1 ELSE 0 END))
                                GROUP BY X.""Id"", X.""DenumireScurta""
                            ORDER BY X.""Id"", X.""DenumireScurta""
...
#region grDate
        ...
        protected void grDate_DataBound(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var rowValues = grid.GetRowValues(i, new string[] { "Cheia", "ValActive", "DrepturiModif", "ValAbsente" }) as object[];
