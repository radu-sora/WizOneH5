...
namespace WizOne.Pagini
{
    public partial class ActeAditionale : System.Web.UI.Page
    {
        ...
        private void IncarcaGrid()
        {
            ...
            {
                string filtru = "";
                if (General.Nz(cmbTip.Value,9).ToString() != "9") filtru = " AND A.\"Candidat\"= " + cmbTip.Value;
                if (cmbAng.Value != null) filtru += @" AND A.""F10003""= " + cmbAng.Value;
                if (cmbStatus.Value != null) filtru += @" AND A.""IdStare"" = " + cmbStatus.Value;
                ...
                dt = General.IncarcaDT($@"SELECT {cmp} AS Cheie, X.*
                        FROM (
                        SELECT A.F10003, COALESCE(B.F10008, '') + ' ' + COALESCE(B.F10009, '') AS NumeComplet, A.DataModif, 0 AS Candidat,
                        ...
                        MAX(CASE WHEN COALESCE(DurataTimpMunca, 0) = 1 THEN 1 ELSE 0 END) AS CIMDet,
                        MAX(CASE WHEN COALESCE(DurataTimpMunca, 0) = 2 THEN 1 ELSE 0 END) AS CIMNed,
                        ...
                        FOR XML PATH ('')) AS IdAvans
                        FROM Avs_Cereri A
                        LEFT JOIN F100 B ON A.F10003 = B.F10003
                        LEFT JOIN Admin_NrActAd J ON A.IdActAd=J.IdAuto
                        WHERE A.IdStare = 3
                        GROUP BY A.F10003, B.F10008, B.F10009, A.DataModif, J.DocNr, J.DocData, COALESCE(J.Tiparit,0), COALESCE(J.Semnat,0), COALESCE(J.Revisal,0), J.IdAuto
                        UNION
                        SELECT A.F10003, COALESCE(A.F10008, '') + ' ' + COALESCE(A.F10009, '') AS NumeComplet, A.F10022, 1 AS Candidat,
                        0, 0, 0, 0, 0, 0, 0, 0, 0, 
        ...
        protected void grDate_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            try
            {
                ...
                if (str != "")
                {
                    ...
                    List<object> lst = grDate.GetSelectedFieldValues(new string[] { "F10003", "DataModif", "DocNr", "IdAutoAct", "IdAvans", "Tiparit", "Semnat", "Revisal", "NumeComplet" });
                        ...
                        case "btnSterge":
                            {
                                ...
                                General.ExecutaNonQuery($@"DELETE FROM ""tblFisiere"" WHERE ""Tabela""=@1 AND ""Id""=@2 AND ""EsteCerere""=0", new object[] { "Admin_NrActAd", obj[3] });
                            }
                            break;
                        case "btnNr":
                            {
                                for (int i = 0; i < lst.Count(); i++)
                                {
                                    try
                                    {
                                        ...
                                        if (Convert.ToInt32(General.Nz(obj[2], 0)) == 0)
                                        {
                                            DataTable dt = General.IncarcaDT($@"INSERT INTO ""Admin_NrActAd""(F10003, ""DocNr"", ""DocData"", ""DataModificare"", USER_NO, TIME) 
                                                ...
                                                new object[] { obj[0], obj[1], Session["UserId"] });
                        ...
                        case "btnTiparit":
                            {
                                ...
                                for (int i = 0; i < lst.Count(); i++)
                                {
                                    try
                                    {
                                        ...
                                        DataTable dt = General.IncarcaDT($@"UPDATE ""Admin_NrActAd"" SET Tiparit=1 WHERE ""IdAuto""=@1", new object[] { obj[3] });
                                        msg += obj[8] + " - " + Dami.TraduCuvant("proces realizat cu succes") + System.Environment.NewLine;
                                ...
                                for (int i = 0; i < lst.Count(); i++)
                                {
                                    try
                                    {
                                        ...
                                        if (Dami.ValoareParam("FinalizareCuActeAditionale") == "1")
                                        {
                                            ...
                                            for (int x = 0; i < dtAvs.Rows.Count; x++)
                                            {
        ...
        protected void grDate_RowUpdating(object sender, DevExpress.Web.Data.ASPxDataUpdatingEventArgs e)
        {
            try
            {
                ...
                object[] obj = grDate.GetRowValues(grDate.FocusedRowIndex, new string[] { "IdAutoAct", "F10003", "DataModif" }) as object[];
                ...
                General.ExecutaNonQuery($@"IF (SELECT COUNT(*) FROM ""Admin_NrActAd"" WHERE ""IdAuto""=@1)=0
                    ...
                    new object[] { obj[0], docNr, docData, obj[1], obj[2], Session["UserId"] });
