        ...
        protected void Page_Init(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Dami.ValoareParam("PontajulAreCC") == "1")
                {
                    ...
                    string frm = "";
                    if (Constante.tipBD == 2) frm = " FROM DUAL";
                    string strSql = $@"SELECT F06204, F06205 FROM F062
                                WHERE  F06204 IN (SELECT F06204 FROM ""relUserCentruCost"" WHERE ""IdUser""={Session["UserId"]})
                                OR (SELECT COUNT(*) FROM ""relUserCentruCost"" WHERE ""IdUser""={Session["UserId"]})=0
                                UNION
                                SELECT 9999, '-----'  {frm}";

                    //DataTable dtCmb = General.IncarcaDT(strSql, null);
                    //GridViewDataComboBoxColumn colCC = (grCC.Columns["F06204"] as GridViewDataComboBoxColumn);
                    //colCC.PropertiesComboBox.DataSource = dtCmb;
                    ...
                    GridViewDataComboBoxColumn colCC = (grCC.Columns["F06204"] as GridViewDataComboBoxColumn);
                    colCC.PropertiesComboBox.DataSource = Session["PtjCC_CentruCost"];
...
#region grDate
        ...
        protected void btnSaveCC_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string f = Dami.ValoareParam("TransferDinPtjCCInPtjGeneral");
                if (f != "")
                {
                    //transferam suma minutelor din CC in Ptj_Intrari
                    List<object> lst = ValoriChei();
                    string strSql = $@"UPDATE Ptj_Intrari SET {f} = (SELECT SUM(NrOre) FROM Ptj_CC WHERE F10003={lst[0]} AND Ziua={General.ToDataUniv(Convert.ToDateTime(lst[1]))} ) WHERE F10003={lst[0]} AND Ziua={General.ToDataUniv(Convert.ToDateTime(lst[1]))} ";
