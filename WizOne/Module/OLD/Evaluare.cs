    ...
    public static class Evaluare
    {
        ...
        public static string InitializareChestionar(List<metaQuizAngajat> arr, int idUser, int userMarca)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < arr.Count; i++)
                {
                    ...
                    strSQL = "delete from \"Eval_RaspunsIstoric\" where \"IdQuiz\" = {0} and F10003 = {1}";
                    strSQL = string.Format(strSQL, arr[i].IdQuiz, arr[i].F10003);
                    General.ExecutaNonQuery(strSQL, null);
                ...
                string sqlObiective = "SELECT * FROM \"Eval_QuizIntrebari\" WHERE \"IdQuiz\" in (" + strSelectIdQuiz.Substring(1) + ") AND \"TipData\" = 23 ";
                    ...
                    for (int i = 0; i < dtObiective.Rows.Count; i++)
                    {
                        if (dtObiective.Rows[i]["PreluareObiective"] != null && dtObiective.Rows[i]["PreluareObiective"].ToString().Length > 0)
                        {
                            for (int j = 0; j < arr.Count; j++)
                            {
                                ...
                                {//din nomenclator
                                    //Florin 2019.01.17
                                    sqlInsertObi = "INSERT INTO \"Eval_ObiIndividualeTemp\" (\"IdObiectiv\", \"Obiectiv\", \"IdActivitate\", \"Activitate\", \"IdQuiz\", F10003, \"Pozitie\", \"IdLinieQuiz\") "
                                        + "SELECT DISTINCT ob.\"IdObiectiv\", ob.\"Obiectiv\", act.\"IdActivitate\", act.\"Activitate\", " + dtObiective.Rows[i]["IdQuiz"].ToString() + ", " + arr[j].F10003.ToString() + ", 1, " + dtObiective.Rows[i]["Id"].ToString()
                                        ...
                                        + " WHERE setAng.\"Id\" = " + arr[j].F10003.ToString() + " AND lista.\"IdLista\" = tmpl.\"IdNomenclator\" and tmpl.\"TemplateId\" =  " + dtObiective.Rows[i]["TemplateIdObiectiv"].ToString()
                                        + " AND tmpl.\"ColumnName\" = 'Obiectiv' "
                                        + " group by ob.\"IdObiectiv\", ob.\"Obiectiv\", act.\"IdActivitate\", act.\"Activitate\" ";

                                    //sqlInsertObi = "INSERT INTO \"Eval_ObiIndividualeTemp\" (\"IdObiectiv\", \"Obiectiv\", \"IdActivitate\", \"Activitate\", \"IdQuiz\", F10003, \"Pozitie\", \"IdLinieQuiz\", \"IdAuto\") "
                                    //    + "SELECT DISTINCT ob.\"IdObiectiv\", ob.\"Obiectiv\", act.\"IdActivitate\", act.\"Activitate\", " + dtObiective.Rows[i]["IdQuiz"].ToString() + ", " + arr[j].F10003.ToString() + ", 1, " + dtObiective.Rows[i]["Id"].ToString()
                                    //    + " , (SELECT COALESCE(MAX(\"IdAuto\"),0) FROM \"Eval_ObiIndividualeTemp\") +row_number() over(order by  " + (Constante.tipBD == 1 ? "1 / 0) " : "\"IdAuto\"")
                                    //    + " FROM \"Eval_ListaObiectiv\" lista "
                                    //    + " JOIN \"Eval_ListaObiectivDet\" listaOb on listaOb.\"IdLista\" = lista.\"IdLista\" "
                                    //    + " JOIN \"Eval_Obiectiv\" ob ON listaOb.\"IdObiectiv\" = ob.\"IdObiectiv\" "
                                    //    + " JOIN \"Eval_ObiectivXActivitate\" act on listaOb.\"IdObiectiv\" = act.\"IdObiectiv\" and listaob.\"IdActivitate\" = act.\"IdActivitate\" "
                                ...
                                else
                                {
                                    ...
                                    {//din obiectivele angajatului
                                        //Florin 2019.01.17
                                        sqlInsertObi = "INSERT INTO \"Eval_ObiIndividualeTemp\" (\"IdObiectiv\", \"Obiectiv\", \"IdActivitate\", \"Activitate\", \"IdQuiz\", F10003, \"Pozitie\", \"IdLinieQuiz\", \"IdAuto\") "
                                            + "SELECT DISTINCT ob.\"IdObiectiv\", ob.\"Obiectiv\", ob.\"IdActivitate\", ob.\"Activitate\", " + dtObiective.Rows[i]["IdQuiz"].ToString() + ", " + arr[j].F10003.ToString() + ", 1, " + dtObiective.Rows[i]["Id"].ToString()
                                            + " FROM \"Eval_ObiIndividuale\" ob "
                                            + " WHERE ob.\"IdPeriod\" = " + dtObiective.Rows[i]["IdPeriod"].ToString() + " AND ob.F10003 = " + arr[j].F10003.ToString()
                                ...
                                if (sqlInsertObi != "")
                                {
                                    General.ExecutaNonQuery("DELETE FROM \"Eval_ObiIndividualeTemp\" WHERE F10003 = " + arr[j].F10003.ToString() + " AND \"IdQuiz\" = " + dtObiective.Rows[i]["IdQuiz"].ToString() + " AND \"IdLinieQuiz\" = " + dtObiective.Rows[i]["Id"].ToString(), null);
                                    General.ExecutaNonQuery(sqlInsertObi, null);
                                }
                ...
                if (dtCompetente != null && dtCompetente.Rows.Count > 0)
                {
                    for (int i = 0; i < dtCompetente.Rows.Count; i++)
                    {
                        if (dtCompetente.Rows[i]["PreluareCompetente"] != null && dtCompetente.Rows[i]["PreluareCompetente"].ToString().Length > 0)
                        {
                            for (int j = 0; j < arr.Count; j++)
                            {
                                string sqlInsertComp = "";

                                if (dtCompetente.Rows[i]["PreluareCompetente"].ToString() == "0")
                                {//din nomenclator
                                    sqlInsertComp = "INSERT INTO \"Eval_CompetenteAngajatTemp\" (\"IdCategCompetenta\", \"CategCompetenta\", \"IdCompetenta\", \"Competenta\", \"IdQuiz\", F10003, \"Pozitie\", \"IdLinieQuiz\", \"IdAuto\") "
                                        + "SELECT DISTINCT comp.\"IdCategorie\", comp.\"DenCategorie\", compDet.\"IdCompetenta\", compDet.\"DenCompetenta\", " + dtCompetente.Rows[i]["IdQuiz"].ToString() + ", " + arr[j].F10003.ToString() + ", 1, " + dtCompetente.Rows[i]["Id"].ToString()
                                        + " , (SELECT COALESCE(MAX(\"IdAuto\"),0) FROM \"Eval_CompetenteAngajatTemp\") +row_number() over(order by  " + (Constante.tipBD == 1 ? "1 / 0) " : "\"IdAuto\")")
                                        ...
                                        + " WHERE setAngDet.\"Id\" = " + arr[j].F10003.ToString() + " AND comp.\"IdCategorie\" = tmpl.\"IdNomenclator\" and tmpl.\"TemplateId\" =  " + General.Nz(dtCompetente.Rows[i]["TemplateIdCompetenta"].ToString(),"-99")
                                ...
                                else
                                {
                                    ...
                                    {//din competentele angajatului
                                        sqlInsertComp = "INSERT INTO \"Eval_CompetenteAngajatTemp\" (\"IdCategCompetenta\", \"CategCompetenta\", \"IdCompetenta\", \"Competenta\", \"IdQuiz\", F10003, \"Pozitie\", \"IdLinieQuiz\", \"IdAuto\") "
                                            + "SELECT DISTINCT comp.\"IdCategCompetenta\", comp.\"CategCompetenta\", comp.\"IdCompetenta\", comp.\"Competenta\", " + dtCompetente.Rows[i]["IdQuiz"].ToString() + ", " + arr[j].F10003.ToString() + ", 1, " + dtCompetente.Rows[i]["Id"].ToString()
                                            + " , (SELECT COALESCE(MAX(\"IdAuto\"),0) FROM \"Eval_CompetenteAngajatTemp\") +row_number() over(order by " + (Constante.tipBD == 1 ? "1 / 0) " : "\"IdAuto\")")
                                            + " FROM \"Eval_CompetenteAngajat\" comp "
                                            + " WHERE comp.\"IdPeriod\" = " + dtCompetente.Rows[i]["IdPeriodComp"].ToString() + " AND comp.F10003 = " + arr[j].F10003.ToString()
                                            + " group by  comp.\"IdCategorie\", comp.\"DenCategorie\", compDet.\"IdCompetenta\", compDet.\"DenCompetenta\" ";
                                    }
                                }
        ...
        public static List<metaEvalDenumireSuper> EvalDenumireSuper(int idQuiz, int pozitie, int consultantRU = 0)
        {
            ...
            try
            {
                ...
                DataTable dtDr = General.IncarcaDT(sqlQuery, null);
                ...
                sqlQuery = @"select * from ""Eval_Circuit"" where ""IdQuiz"" = {0}";
                sqlQuery = string.Format(sqlQuery, idQuiz);
                DataTable dtCir = General.IncarcaDT(sqlQuery, null);
                        ...
                        if (piCir != null)
                        {
                            ...
                            if (ok)
                            {
                                ...
                                if (id == 0)
                                {
                                    ...
                                    string idCateg = General.Nz(General.ExecutaScalar(@"SELECT ""CategorieQuiz"" FROM ""Eval_Quiz"" WHERE ""Id""=" + idQuiz, null),"0").ToString();
                                ...
                                else
                                {
                                    if (id < 0)
                                    {
                                        ...
                                        strSuper = @"select * from ""tblSupervizori"" where ""Id"" = -1 * {0}";
                                        strSuper = string.Format(strSuper, id);
                                        DataTable dtSuper = General.IncarcaDT(strSuper, null);
