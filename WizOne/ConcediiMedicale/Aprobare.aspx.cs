    ...
    public partial class Aprobare : System.Web.UI.Page
    {
        ...
        protected void btnAnulare_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string sql = "UPDATE CM_CereriIstoric SET Aprobat = 1, DataAprobare = GETDATE(), CULOARE = (SELECT Culoare FROM CM_tblStari WHERE Id = -1), IdUser = " + Session["UserId"].ToString() + " WHERE IdCerere = " + ids[i].Id + " AND Pozitie = 2";
                    General.ExecutaNonQuery(sql, null);
        ...
        bool AddConcediu(int cod, int zile, int cc, double proc, double suma_4450_subplafon, bool bFTarif, int marca, bool avans)
        {
            ...
            // 13           14                                                  15                                      16                                          17                                          18                                                                                               19                                                                                         20
             ...
             BCCM, ZileBCCM, MZCM.Replace(',', '.'), dt.Rows[0]["NrAvizMedicExpert"].ToString(), dt.Rows[0]["MedicCurant"].ToString(), (dt.Rows[0]["CNPCopil"] == DBNull.Value ? "" : dt.Rows[0]["CNPCopil"].ToString()), MNTZ.Replace(',', '.'),
