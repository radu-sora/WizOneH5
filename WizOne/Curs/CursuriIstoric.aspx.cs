        ...
        public DataTable GetCursIstoric(int f10003, int idCurs, int idSesiune, string niv1, string niv2, string divizie, string dept, int idUser, int marcaMea)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    string idManager = Dami.ValoareParam("Curs_IDuriManager", "1");
                    strFiltru += "AND (X.F10003 = " + marcaMea + " OR X.F10003 IN (SELECT F10003 FROM \"F100Supervizori\" WHERE \"IdUser\"=" + idUser + " AND \"IdSuper\" IN (" + idManager + ")) OR (SELECT COUNT(*) FROM Curs_tblCursSesiune a LEFT JOIN Curs_tblTraineri b on  a.TrainerId=b.id where b.iduser =" + idUser + ")>0)";
                }
