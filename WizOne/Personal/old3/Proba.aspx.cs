...
namespace WizOne.Personal
{
    public partial class Proba : System.Web.UI.Page
    {
        ...
        private void IncarcaCtl()
        {
            try
            {
                var edc = FindControlRecursive(this,"ctl_F10022");

                string ttt = "";
                for(int x=0; x < tabMP.Page.Controls.Count; x++)
                {
                    ttt += this.Master.Controls[x].ID;

                }
                

                DataTable dtF100 = General.IncarcaDT("SELECT * FROM F100 WHERE F10003=460", null);
                if (dtF100 != null && dtF100.Rows.Count > 0)
                {
                    DataRow dr = dtF100.Rows[0];
                    Page pag = this.Page;
                    var ert = pag.FindControl("ctl_F10022");
                    //for(int i =0; i < )
                }
